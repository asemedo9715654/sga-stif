...
using sga_stif.ViewModel.Breadcrumb;
...
namespace sga_stif.Filtros
{
  public class BreadcrumbActionFilter : ActionFilterAttribute
  {
    ...
    private List<BreadcrumbViewModel> ConfigureBreadcrumb(ActionExecutedContext context)
    {
      var breadcrumbList = new List<BreadcrumbViewModel>();
      var homeControllerName = "Socio";
       var outro = "ListaSocio";

      breadcrumbList.Add(new BreadcrumbViewModel
      {
        Text = "Lista de Sócio",
        Action = "ListaSocio",
      ...
      if (context.HttpContext.Request.Path.HasValue)
      {
        ...
        for (var i = 0; i < pathSplit.Length; i++)
        {
          ...
          if (string.IsNullOrEmpty(pathSplit[i]) || string.Compare(pathSplit[i], homeControllerName, true) == 0 || string.Compare(pathSplit[i], outro, true) == 0)
          {
            continue;
          }
          ...
          if (controller != null)
          {
            var indexMethod = controller.GetMethod("ListaSocio");
            ...
            if (indexMethod != null)
            {
              ...
              if (i + 1 < pathSplit.Length && string.Compare(pathSplit[i + 1], "ListaSocio", true) == 0)
              {
    ...
    private Type GetControllerType(string name)
    {
      ...
      try
      {
        controller = Assembly.GetCallingAssembly().GetType("sga_stif.Controllers." + name);
      }
      catch
      { }
